...
import {ValidatorSet, Validator} from "../../contracts/ValidatorSet.sol";
...
contract ValidatorSetTest is BaseTest  {
	...
	function setUp() public override {
        BaseTest.setUp();
        address addr = address(s_allContracts.validatorSet);
        s_validatorSet = ValidatorSet(addr);
	}
    ...
    function test_construction() public {
        ...
        // Validator storage asdsa = s_validatorSet.currentValidatorSet(0);//.consensusAddress;

        Validator memory val = s_validatorSet.getValidator(0);
        
        address xxx = val.operateAddress;
        console.log("xxx :   %s", xxx);

        // Validator memory aa = s_validatorSet.currentValidatorSet(0);
