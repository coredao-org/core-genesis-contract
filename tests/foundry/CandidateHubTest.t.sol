...
contract CandidateHubTest is BaseTest  {
	...
	function setUp() public override {
	    BaseTest.setUp();
        s_candidateHub = CandidateHub(address(s_allContracts.candidateHub));
	}
    ...
    function testFuzz_jailValidator(uint32 commissionThousandths, uint value, uint256 round, uint256 fine) public {
        ...
        _hoaxWithGas(address(s_allContracts.validatorSet)); // only validator may call jailValidator()
        s_candidateHub.jailValidator(candidate, round, fine);
    }

    // function testFuzz_turnRound(uint validatorSetBalance) public {zzzzz
    ...
    //     vm.deal(address(s_allContracts.validatorSet), validatorSetBalance);
