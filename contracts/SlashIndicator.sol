...
contract SlashIndicator is ISlashIndicator,System,IParamSubscriber{
  ...
  function slash(address validator) external onlyCoinbase onlyInit oncePerBlock onlyZeroGasPrice{
    ...
    address operator = IValidatorSet(VALIDATOR_CONTRACT_ADDR).getOperator(validator);
    IPledgeAgent(PLEDGE_AGENT_ADDR).onSlash(operator, indicator.count, misdemeanorThreshold, felonyThreshold);
