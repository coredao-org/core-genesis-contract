...
contract BitcoinStake is IBitcoinStake, System, IParamSubscriber, ReentrancyGuard {
  ...
  function delegate(bytes calldata btcTx, uint32 blockHeight, bytes32[] memory nodes, uint256 index, bytes memory script) external override nonReentrant {
    ...
    uint64 btcAmount;
    {
      ...
      uint32 outputIndex;
      uint256 fee;
      (btcAmount, outputIndex, delegator, candidate, fee) = _parseVout(voutView, script);
  ...
  function _parseVout(
    ...
    for (uint index = 0; index < _numberOfOutputs; index++) {
      ...
      if(_arbitraryData == TypedMemView.NULL) {
          ...
          (delegator, candidate, fee) = _parsePayloadAndCheckProtocol(_arbitraryData);
          opreturn = true;
      }
    }
  ...
  function _parsePayloadAndCheckProtocol(bytes29 payload) internal pure returns (address delegator, address candidate, uint256 fee) {
