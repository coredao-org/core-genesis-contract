...
interface IBitcoinStake {
  ...
  function distributeReward(address[] calldata validators, uint256[] calldata rewardList, uint256 round) external payable;
  function getStakeAmounts(address[] calldata candidators) external view returns (uint256[] memory amounts);
