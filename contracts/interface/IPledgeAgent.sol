...
interface IPledgeAgent {
  ...
  function getHybridScore(address[] memory candidates, bytes20[] memory lastMiners, bytes20[] memory miners, uint256[] memory powers) external returns(uint256[] memory, uint256, uint256);
