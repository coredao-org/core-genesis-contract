...
contract PledgeAgent is IPledgeAgent, System, IParamSubscriber {
  ...
  uint256 public constant INIT_UNDELEGATE_LOCK_TURNS = 3;
  ...
  mapping(address => uint128[]) public undelegateUnlockMap;
  ...
  function undelegateCoin(address agent, uint256 amount) public {
    ...
    uint128[] storage list = undelegateUnlockMap[msg.sender];
    ...
    if (len != 0 && (list[len-1] >> 96) == unlockRound) {
      list[len-1] += uint128(deposit);
    } else {
      list.push(uint128((unlockRound << 96) | deposit));
    }
  ...
  function claimReward(address[] calldata agentList) external returns (uint256, bool) {
    ...
    rewardSum += collectLockDeposit(msg.sender);
    if (rewardSum != 0) {
  ...
  function undelegateCoin(address agent, address delegator, uint256 amount) internal returns (uint256) {
    ...
    bool haveReward;
    if (a.rewardSet.length != 0) {
      haveReward = a.rewardSet[a.rewardSet.length - 1].round == roundTag;
    }
    if (!haveReward && newDeposit == amount) {
  ...
  function collectLockDeposit(address delegator) public returns (uint256) {
    uint256 rewardSum;
    uint128[] storage list = undelegateUnlockMap[delegator];
    uint256 len = list.length;
    uint256 curRound = roundTag;
    bool del = len > 0;
    for (uint256 i = len; i > 0; --i) {
      uint128 pair = list[i - 1];
      if (pair != 0) {
        uint256 round = pair >> 96;
        uint256 deposit = pair & (2**96-1);
        if (round < curRound) {
          del = false;
        } else {
          rewardSum += deposit;
