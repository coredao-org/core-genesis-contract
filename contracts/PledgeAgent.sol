...
contract PledgeAgent is IPledgeAgent, System, IParamSubscriber {
  ...
  /* key: delegator’s fee address, or eth formated address of btc miner
   * value: btc delegator
   */
  mapping(address => BtcDelegator) public btcDelegatorsMap;
  /* key: btc compressed/uncompressed pk hash,
   * value: eth formated address of btc miner
   */
  mapping(bytes20 => address) public btc2ethMap;
  /* key: round index
   * value: key information state of round
   */
  ...
  // When a reward received by delegators,
  // there is some dust leave in remainReward.
  ...
  struct Reward {
    ...
    uint256 totalIntegral; // calc each delegator's reward which state
    uint256 coin;
    uint256 round;
  }
  ...
  struct Agent {
    ...
    Reward[] rewardSet;
    // round State of lastest validator
    uint256 power;
    uint256 coin;
  }
  ...
  /**
   * add reward by validator set. agentList is operator address list
   */
  ...
  function getIntegral(
    ...
    // collect power reward, reset delegator's power & agent's power、coin
    uint256 reward = 0;
    ...
    for (uint256 i = 0; i < miners.length; i++) {
      ...
      if (delegator != address(0x0)) {
        // in order to deduce denominator deviation, mul 1e18 for each power
  ...
  function claimReward(address payable delegator, address[] calldata agentList) external returns (uint256, bool) {
    // In order to avoid the gas out case, limit the round count.
  ...
  function collectCoinReward(
    ...
    // update index when any one reward collected.
  ...
  function gatherDust() external onlyInit onlyGov {
    if (totalDust > 0) {
      address payable govhub = address(uint160(GOV_HUB_ADDR));
      govhub.transfer(totalDust);
      totalDust = 0;
    }
  }
