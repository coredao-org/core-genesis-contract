...
contract ValidatorSet is IValidatorSet, System, IParamSubscriber {
  ...
  uint256 public constant INIT_VOTE_REWARD_PERCENT = 10;
  ...
  function vote(address[] calldata valAddrs) external payable onlyCoinbase onlyInit onlyZeroGasPrice {
    for (uint256 i; i < valAddrs.length; ++i) {
      uint256 index = currentValidatorSetMap[valAddrs[i]];
      if (index > 0) {
        currentValidatorSet[index - 1].voteWeight += 1;
      }
    }
  }
  ...
  function distributeReward() external override onlyCandidate returns (address[] memory operateAddressList) {
    ...
    if (voteRewardPercent == 0) {
      voteRewardPercent = INIT_VOTE_REWARD_PERCENT;
    }
    for (uint256 i; i < validatorSize; i++) {
    ...
    if (voteWeightSum != 0) {
      totalVoteReward = incentiveSum * voteRewardPercent / 100;
      totalVoteReward -= totalVoteReward % voteWeightSum;
    }
    ISystemReward(SYSTEM_REWARD_ADDR).receiveRewards{ value: incentiveSum-totalVoteReward }();
    ...
    for (uint256 i; i < validatorSize; i++) {
      ...
      if (tempIncome != 0) {
        ...
        voteReward = voteWeightSum == 0 ? 0 : v.voteWeight * totalVoteReward / voteWeightSum;
        ...
        v.income = 0;
        v.voteWeight = 0;
        bool success = feeAddress.send(validatorReward+voteReward);
        if (success) {
          emit directTransfer(v.operateAddress, feeAddress, validatorReward, tempIncome, voteReward);
        } else {
          emit directTransferFail(v.operateAddress, feeAddress, validatorReward, tempIncome, voteReward);
        }
      }
    }
