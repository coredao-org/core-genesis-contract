...
interface IPledgeAgentMock {
 function addRoundReward(address[] calldata agentList, uint256[] calldata rewardList) payable external;
  ...
  function distributePowerRewardOld(address candidate, address[] calldata miners) external;
  function getHybridScoreOld(address[] calldata candidates, uint256[] calldata powers, uint256 round) external returns(uint256[] memory);
  function setNewRoundOld(address[] calldata validatorList, uint256 round) external;
  function addRoundRewardOld(address[] calldata agentList, uint256[] calldata rewardList) payable external;
}
